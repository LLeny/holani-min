...
use clap::Parser;
use holani::{cartridge::lnx_header::LNXRotation, mikey::video::{LYNX_SCREEN_HEIGHT, LYNX_SCREEN_WIDTH}, suzy::registers::{Joystick, Switches}};
use keycodes::translate_keycode;
use miniquad::window::screen_size;
use macroquad::prelude::*;
use runner::{runner_config::{Input, RunnerConfig}, Runner};
...
async fn main() {
    ...
    loop {
        ...
        config.button_mapping().iter().for_each(|btn| 
            match *btn.1 {Input::Pause=>switches.set(Switches::pause,is_key_down(*btn.0)),
